...
async function handleRequest(request) {
  ...
  try {
    ...
    for (const entry of unique.values()) {
      ...
      const isPureMV =
        MV_KEYWORDS.some((kw) => titleUpper.includes(kw)) && !isKaraoke;
      if (isPureMV) {
        continue;
      }
    ...
    const res = await fetch(
      `https://www.youtube.com/youtubei/v1/search?key=${YT_KEY}`,
      {
        ...
        headers: {
          ...
          "User-Agent":
            "com.google.android.youtube/19.26.35 (Linux; U; Android 11; gzip)",
        },
        body: JSON.stringify({
          context: {
            client: {
    ...
    if (res.ok) {
      ...
      walk(data, (node) => {
        if (node && (node.videoRenderer || node.compactVideoRenderer)) {
          ...
          const channel = getRunsText(
            v.ownerText || v.shortBylineText || v.longBylineText
          );
          const duration =
            v.lengthText?.simpleText || getRunsText(v.lengthText) || "";
